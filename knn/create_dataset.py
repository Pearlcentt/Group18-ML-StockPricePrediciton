...
def create(*, code: str = pr.code,
    ...
    logger.info(
        f"Create dataset object with parameters: {dict(seq_length=seq_length, trainsforms=transform.__name__ if transform is not None else None, prediction_step=prediction_step, profit_rate=profit_rate)}")
    ...
    logger.info(
        f"Create Subset and DataLoader objects with {split_func.__name__} using split_ratio = {split_ratio} and batch_size = {batch_size}")
...
data = data.flatten() if data.shape[-1] == 1 else data
profit = [(data[idx + pr.prediction_step] / data[idx]) - 1 for idx in range(len(data) - pr.prediction_step)]
profit_rate = np.median(profit) if pr.use_median else pr.profit_rate
dataset = UpDownStockDataset(data=data,
                                 seq_length=pr.seq_length,
                                 transforms=pr.transform,
                                 prediction_step=pr.prediction_step,
                                 profit_rate=float(profit_rate))
test_dataloader = DataLoader(dataset=dataset,batch_size=pr.batch_size,drop_last=False,shuffle=False)
