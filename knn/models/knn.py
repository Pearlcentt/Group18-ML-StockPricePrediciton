...
class KNearestNeighbors():
    def __init__(self, x: np.ndarray, y: np.ndarray,k: int, similarity: str = "euclid", device="cpu"):
        if device == "cuda":
            self.CUDA = True
        else:
            self.CUDA = False
    ...
    def _euclid(self, x1, x2):
        ...
        if self.CUDA:  # (x1 - x2).shape: batch_size, num_samples, num_features
            return torch.norm(torch.from_numpy(x1).to(self.device) - torch.from_numpy(x2).to(self.device),
                              dim=-1).cpu().numpy()
        return np.linalg.norm(x1 - x2, axis=-1)

    def _manhattan(self, x1, x2):
        if self.CUDA:
            return torch.abs(torch.from_numpy(x1).to(self.device) - torch.from_numpy(x2).to(self.device)).sum(
                -1).cpu().numpy()
    ...
    def _cosine(self, x1, x2):
        ...
        return 1 - torch.nn.functional.cosine_similarity(torch.from_numpy(x1).to(self.device),
                                                         torch.from_numpy(x2).to(self.device), dim=-1).cpu().numpy()
...
class WeightedKNearestNeighbors:
    def __init__(self, x: np.ndarray, k: int, y: np.ndarray, similarity: str = "euclid", weights=1, learning_rate=0.01,
                 device="cpu", train_split_ratio=0.7):
